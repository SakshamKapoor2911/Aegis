...
# GKE Security Agent Docker Deployment Script
# Fallback script using local Docker when Cloud Build fails
...
cd backend
docker build -t $REGION-docker.pkg.dev/$PROJECT_ID/gke-security-agent/backend .
docker push $REGION-docker.pkg.dev/$PROJECT_ID/gke-security-agent/backend
...
cd ../frontend
docker build -t $REGION-docker.pkg.dev/$PROJECT_ID/gke-security-agent/frontend .
docker push $REGION-docker.pkg.dev/$PROJECT_ID/gke-security-agent/frontend
